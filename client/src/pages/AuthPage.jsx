...
import { BookOpen, User, Eye, EyeOff, Lock, Mail, ChevronLeft, BookMarked, RefreshCw, Video, Smartphone, GraduationCap, Library } from 'lucide-react';
import { useAuth } from '../context/AuthContext';

const AuthPage = () => {
  const navigate = useNavigate();
  const { login, register, loginWithGoogle } = useAuth();
  const [loginRole, setLoginRole] = useState('student'); // 'student' or 'librarian'
  ...
  const handleRoleSwitch = (role) => {
    setLoginRole(role);
    setIsLogin(true); // Always reset to login when switching role
    setError('');
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isLogin) {
        ...
        // Always redirect based on the actual role from DB
  ...
  return (
    <div className="min-h-screen flex text-slate-800 dark:text-slate-100 bg-slate-50 dark:bg-slate-900">
      ...
      <div className="hidden lg:flex w-1/2 relative overflow-hidden">
        ...
        <div className="absolute inset-0 overflow-hidden">
          ...
          <div className="animate-float-delayed absolute top-[60%] left-[15%] w-24 h-24 rounded-2xl bg-amber-400/20 backdrop-blur-sm border border-amber-400/30 -rotate-6" />
          <div className="animate-float-slow absolute top-[25%] right-[10%] w-40 h-40 rounded-full bg-blue-400/10 backdrop-blur-sm border border-blue-300/20" />
          <div className="animate-float absolute bottom-[15%] right-[20%] w-28 h-28 rounded-2xl bg-white/10 backdrop-blur-sm border border-white/20 rotate-45" />
          <div className="animate-float-delayed absolute top-[45%] left-[50%] w-16 h-16 rounded-xl bg-amber-300/15 backdrop-blur-sm border border-amber-300/20 rotate-12" />
          
          {/* Book icon decorations */}
          <div className="animate-float-slow absolute top-[15%] right-[25%] text-white/20">
            <BookOpen className="w-20 h-20" />
          </div>
          <div className="animate-float absolute bottom-[30%] left-[30%] text-white/15">
        ...
        <div className="relative z-10 flex flex-col justify-between p-12 text-white w-full">
          <div>
            <Link to="/" className="flex items-center gap-2 mb-16 hover:opacity-80 transition-opacity w-fit text-white/80 hover:text-white">
            ...
            <div className="flex items-center gap-4 mb-8">
              ...
              <div>
                ...
                <p className="text-blue-200 text-lg">Digital Academic Library</p>
              </div>
            </div>
          ...
          <div className="mt-12 p-6 rounded-2xl bg-white/10 backdrop-blur-sm border border-white/20">
            {loginRole === 'librarian' ? (
              <>
                <p className="text-blue-100 italic mb-3">"The renewal video consultation feature saves so much time. I can manage everything from the dashboard!"</p>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-blue-200 text-xs">Head Librarian, CUET</p>
                  </div>
                </div>
              </>
            ) : (
              <>
                <p className="text-blue-100 italic mb-3">"CUET Bookworld made finding and borrowing academic books so easy. I can manage everything from my phone!"</p>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-blue-200 text-xs">CSE, 3rd Year</p>
                  </div>
                </div>
              </>
            )}
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-6 sm:p-12 relative">
        ...
        <div className="w-full max-w-md space-y-6">
          ...
          <div className="lg:hidden flex items-center justify-center gap-2 mb-4 pt-8">
            ...
            <span className="text-2xl font-bold text-slate-800 dark:text-white">CUET Bookworld</span>
          </div>
          ...
          <div className="bg-white dark:bg-slate-800 p-8 sm:p-10 rounded-2xl shadow-xl border border-slate-100 dark:border-slate-700">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <>
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input 
                        ...
                        className="block w-full pl-10 pr-3 py-2.5 border border-slate-300 dark:border-slate-600 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-transparent text-sm" 
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <input 
                        type="text" name="studentId" required onChange={handleChange}
                        className="block w-full px-3 py-2.5 border border-slate-300 dark:border-slate-600 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-transparent text-sm" 
                    ...
                    <div>
                      ...
                      <select name="department" onChange={handleChange} className="block w-full px-3 py-2.5 border border-slate-300 dark:border-slate-600 rounded-xl focus:ring-2 focus:ring-blue-500 bg-transparent dark:bg-slate-800 text-sm">
                        {['CSE', 'EEE', 'CE', 'ME', 'URP', 'ECE', 'MSE', 'PME', 'WRE', 'Architecture'].map(dept => (
                  ...
                  <div>
                    ...
                    <select name="year" onChange={handleChange} className="block w-full px-3 py-2.5 border border-slate-300 dark:border-slate-600 rounded-xl focus:ring-2 focus:ring-blue-500 bg-transparent dark:bg-slate-800 text-sm">
                      {['1st', '2nd', '3rd', '4th', 'Faculty'].map(y => (
                        <option key={y} value={y}>{y} {y !== 'Faculty' ? 'Year' : ''}</option>
                      ))}
                    </select>
                  </div>
                </>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input 
                    ...
                    className="block w-full pl-10 pr-3 py-2.5 border border-slate-300 dark:border-slate-600 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-transparent text-sm" 
                    placeholder={loginRole === 'librarian' ? 'librarian@cuet.ac.bd' : 'student@cuet.ac.bd'} />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input 
                    ...
                    className="block w-full pl-10 pr-10 py-2.5 border border-slate-300 dark:border-slate-600 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-transparent text-sm" 
                    placeholder="••••••••" />
                  <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute inset-y-0 right-0 pr-3 flex items-center text-slate-400 hover:text-slate-600 cursor-pointer">
              ...
              {isLogin && (
                <div className="flex items-center justify-between text-sm">
                  ...
                  <a href="#" className="text-blue-600 dark:text-blue-400 hover:underline font-medium">Forgot password?</a>
                </div>
              )}
              ...
              <button
                type="submit"
                disabled={loading}
                className={`w-full flex justify-center py-3 px-4 rounded-xl text-sm font-semibold text-white focus:outline-none focus:ring-2 focus:ring-offset-2 transition-colors disabled:opacity-60 disabled:cursor-not-allowed shadow-sm ${
                  loginRole === 'librarian'
            ...
            {loginRole === 'student' && (
              <>
                ...
                <button
                  ...
                  disabled={loading}
                  className="w-full flex items-center justify-center gap-3 py-2.5 px-4 rounded-xl border border-slate-300 dark:border-slate-600 text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors text-sm font-medium disabled:opacity-60"
                >
            ...
            {/* Toggle Login/Register — only for students */}
            {loginRole === 'student' && (
